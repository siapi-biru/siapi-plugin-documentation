...
module.exports = {
  ...
  createDocumentationFile: function(apiName, writeFile = true) {
    ...
    return Object.keys(apiDocumentation).reduce((acc, docName) => {
      ...
      const components =
        siapi.models[docName] !== undefined ? this.generateResponseComponent(docName) : {};
  ...
  createPluginDocumentationFile: function(pluginName, writeFile = true) {
    ...
    return Object.keys(pluginDocumentation).reduce((acc, docName) => {
      ...
      const components =
        _.get(siapi, this.getModelForPlugin(docName, pluginName)) !== undefined &&
        pluginName !== 'upload'
  ...
  deleteDocumentation: async function(version = this.getDocumentationVersion()) {
    ...
    apis.forEach(api => {
      const apiPath = path.join(siapi.config.appPath, 'api', api, 'documentation', version);
    ...
    return Object.keys(attributes).reduce(
      (acc, curr) => {
        ...
        if (isField) {
          ...
          const relationNature = _.get(siapi, newGetter).filter(
  ...
  generateFullDoc: function(version = this.getDocumentationVersion()) {
    ...
    const defaultSettings = _.cloneDeep(
      _.pick(siapi.plugins.documentation.config, defaultSettingsKeys)
    );
    ...
    return Object.keys(attributes).reduce(
      (acc, current) => {
        ...
        if (attribute.model || attribute.collection) {
          ...
          const associationAttributes = _.get(siapi, getter);
          ...
          const cmp = this.generateMainComponent(
            siapi.components[component].attributes,
            siapi.components[component].associations
          );
  ...
  generatePluginVerbResponses: function(routeObject) {
    ...
    const { generateDefaultResponse } = siapi.plugins.documentation.config['x-siapi-config'];
  ...
  generateResponseComponent: function(tag, pluginName = '', isPlugin = false) {
    ...
    const attributesObject = _.get(siapi, attributesGetter);
    const privateAttributes = this.getPrivateAttributes(attributesObject);
    const modelAssociations = _.get(siapi, associationGetter);
    ...
    // Please refer https://github.com/siapi/siapi/blob/585800b7b98093f596759b296a43f89c491d4f4f/packages/siapi/lib/middlewares/mask/index.js#L92-L100
  ...
  generateResponseDescription: function(verb, tag, endPoint) {
    const isModelRelated = siapi.models[tag] !== undefined && tag === endPoint;
  ...
  generateResponseSchema: function(verb, routeObject, tag) {
    ...
    if (shouldCheckIfACustomEndPointReferencesAnotherModel) {
      ...
      isModelRelated = _.get(siapi, getter) !== undefined && isKnownAction;
      ...
      isModelRelated = siapi.models[tag] !== undefined && tag === _.lowerCase(controller);
    }
  ...
  generateVerbParameters: function(verb, controllerMethod, endPoint) {
    ...
    if (verb === 'get' && controllerMethod === 'find') {
      ...
      // of the documentation https://siapi.io/documentation/developer-docs/latest/developer-resources/content-api/content-api.html#filters
  ...
  getApiRoutes: apiName => {
    return _.get(siapi, ['api', apiName, 'config', 'routes'], []);
  },
  ...
  getFullDocumentationPath: () => {
    return path.join(siapi.config.appPath, 'extensions', 'documentation', 'documentation');
  },
  ...
  getDocumentationVersion: () => {
    const version = siapi.plugins['documentation'].config.info.version;
  ...
  getPluginsWithDocumentationNeeded: function() {
    return Object.keys(siapi.plugins).reduce((acc, current) => {
  ...
  getPluginRoutesWithDescription: function(pluginName) {
    return _.get(siapi, ['plugins', pluginName, 'config', 'routes'], []).filter(
  ...
  isPluginDocumentationNeeded: function(pluginName) {
    const { pluginsForWhichToGenerateDoc } = siapi.plugins.documentation.config['x-siapi-config'];
    if (
  ...
  retrieveDocumentation: function(name, isPlugin = false) {
    const documentationPath = isPlugin
      ? [siapi.config.appPath, 'extensions', name, 'documentation', this.getDocumentationVersion()]
      : [siapi.config.appPath, 'api', name, 'documentation', this.getDocumentationVersion()];
  ...
  retrieveDocumentationFiles: function(isPlugin = false, version = this.getDocumentationVersion()) {
    ...
    return array.reduce((acc, current) => {
      const documentationPath = isPlugin
        ? [siapi.config.appPath, 'extensions', current, 'documentation', version]
        : [siapi.config.appPath, 'api', current, 'documentation', version];
