...
module.exports = {
  getInfos: async ctx => {
    try {
      const service = siapi.plugins.documentation.services.documentation;
      ...
      ctx.send({
        ...
        prefix: siapi.plugins.documentation.config['x-siapi-config'].path,
        form,
      });
    } catch (err) {
  ...
  async index(ctx, next) {
    try {
      ...
      const version =
        major && minor && patch
          ? `${major}.${minor}.${patch}`
          : siapi.plugins.documentation.config.info.version;
      ...
      try {
        ...
        const filledLayout = _.template(layout)({
          backendUrl: siapi.config.server.url,
  ...
  async loginView(ctx, next) {
    ...
    try {
      ...
      const filledLayout = _.template(layout)({
        actionUrl: `${siapi.config.server.url}${siapi.plugins.documentation.config['x-siapi-config'].path}/login`,
      });
  ...
  async login(ctx) {
    ...
    const { password: storedPassword } = await siapi
      .store({
        environment: '',
        type: 'plugin',
        name: 'documentation',
    ...
    ctx.redirect(
      `${siapi.config.server.url}${siapi.plugins.documentation.config['x-siapi-config'].path}${querystring}`
    );
  },
  ...
  updateSettings: async ctx => {
    ...
    const usersPermService = siapi.plugins['users-permissions'].services;
